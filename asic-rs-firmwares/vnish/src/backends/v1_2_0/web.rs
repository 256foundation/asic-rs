...
impl VnishWebAPI {
    ...
    ///
    /// VNish 1.2.x expects this request without a body and reports the delay
    /// before its background restore task reboots the miner.
    pub async fn restore_stock_os(&self) -> anyhow::Result<u64> {
        let response = self
            .send_command("firmware/remove", true, None, Method::POST)
            .await?;
